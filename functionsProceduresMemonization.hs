...
assignArray (A xs) (I i) (I v) = A ((i,v) : filter ((i/=).fst) xs)
...
applyOp Index (A xs) (I n) = if (null v) then (I 0) else (I (head v))
  where
  (_,v) = unzip(filter ((n==).fst) xs)
...
evalArgs exs defs s = [eval x defs s | x<-exs]
...
eval (Cond p e1 e2) fd s = 
  if ((eval p fd s) == (I 1)) then (eval e1 fd s) else (eval e2 fd s)
eval (OpApp o e1 e2) fd s =
  applyOp o (eval e1 fd s) (eval e2 fd s)
eval (FunApp id es) fd s = eval e fd (bindArgs as vs ++ s)
  where
  (as,e) = lookUp id fd
  vs     = evalArgs es fd s 
...
executeStatement (Assign id ex) fds _ state = 
  updateVar (id, (eval ex fds state)) state
